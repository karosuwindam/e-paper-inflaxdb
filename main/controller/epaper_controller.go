...
import (
	...
	epaperv2 "epaperifdb/controller/epaper-v2"
...
func ePaperUpdate(ctx context.Context) error {
	...
	go func(ctx context.Context) {
		...
		data1day <- tmp
		type api struct {
			ClearScreen func(context.Context)
			TextPut     func(context.Context, int, []string, float64) error
		}
...
func ePaperUpdatev2(ctx context.Context) error {
	ctx, span := config.TracerS(ctx, "ePaperUpdate", "e-paper update")
	defer span.End()
	slog.DebugContext(ctx, "ePaper Update Start")
